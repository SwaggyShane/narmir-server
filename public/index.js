...
async function processAiTurns(db) {
  ...
  for (const player of aiPlayers) {
    const k = await db.get('SELECT * FROM kingdoms WHERE player_id = ?', [player.id]);
    if (!k || k.turns_stored < 1) continue;

    // Simple AI strategy: spend all stored turns
    const turnsToSpend = Math.min(k.turns_stored, 5);
    for (let i = 0; i < turnsToSpend; i++) {
      const freshK = await db.get('SELECT * FROM kingdoms WHERE id = ?', [k.id]);
      if (!freshK || freshK.turns_stored < 1) break;

      const { updates, events } = engine.processTurn(freshK);
      updates.turns_stored = freshK.turns_stored - 1;

      // AI build strategy — allocate engineers to farms and housing
      const totalEng = freshK.engineers || 0;
      if (totalEng > 0) {
        const farmEng    = Math.floor(totalEng * 0.4);
        const housingEng = Math.floor(totalEng * 0.3);
        const schoolEng  = Math.floor(totalEng * 0.15);
        const barracksEng= totalEng - farmEng - housingEng - schoolEng;
        updates.build_allocation = JSON.stringify({
          farms: farmEng, housing: housingEng,
          schools: schoolEng, barracks: barracksEng
        });
      }

      // AI research strategy — spread evenly
      if (freshK.researchers > 0) {
        const perDisc = Math.floor(freshK.researchers / 10);
        updates.research_allocation = JSON.stringify({
          economy: perDisc, weapons: perDisc, armor: perDisc,
          military: perDisc, spellbook: 0, attack_magic: perDisc,
          defense_magic: perDisc, entertainment: perDisc,
          construction: perDisc, war_machines: perDisc,
        });
      }

      // Apply updates
      const VALID = ['gold','mana','land','population','morale','food','turn','turns_stored',
        ...
        'build_allocation','research_allocation','build_queue','build_progress','xp','level',
      ];
      const safe = Object.fromEntries(Object.entries(updates).filter(([c,v]) =>
        VALID.includes(c) && v !== undefined && v !== null
      ));
      if (Object.keys(safe).length > 0) {
        const cols = Object.keys(safe).map(c => `${c} = ?`).join(', ');
        await db.run(`UPDATE kingdoms SET ${cols} WHERE id = ?`, [...Object.values(safe), k.id]);
      }
      await engine.resolveExpeditions(db, { ...freshK, ...updates }, engine);
    }
  }
  console.log(`[ai] Processed turns for ${aiPlayers.length} AI kingdoms`);
}
